...
// this function will parse time strings like "2:30 PM" or "14:30" into an object {hour, minute, ampm}
...
// This function will build the base URL for the frontend from the request headers or environment variables
...
// This function will resolve the user ID from the request using multiple strategies
  ...
  try {
    ...
    if (!clerkUserId) {
      return res.status(401).json({
        success: false,
        message: "Authentication required to create an appointment."
      });
    }
...
// to confirn the service payment
export const confirmServicePayment = async (req, res) => {
  ...
  // To getServiceAppointments details
...
// for filtering, we will allow filtering by serviceId, mobile, status and a general search that looks into patientName, mobile and notes
    const filter = {};
  ...
  // We can also add some global stats like total appointments, total earnings etc.
  return res.json({
      success: true,
      services,
  ...
  // to get the appointment for the patient

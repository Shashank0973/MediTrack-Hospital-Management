...
// HELPERS
// this function will return an infinite number
const safeNumber = (v) => {
  const n = Number(v);
  return Number.isFinite(n) ? n : null;
};
// This function will create the function URL
...
export const getAppointmentsByPatient = async(req,res) => {
  ...
  .sort({ date: 1, time: 1 })
  .populate("doctorId", "name image imageUrl")
  .lean();
    return res.json({
...
export const createAppointment = async (req,res) =>  {
  try {
    ...
    const numericFee = safeNumber(fee ?? fees?? 1);
    ...
    const successUrl = `${frontBase}/appointment/success?sessionId={CHECKOUT_SESSION_ID}`;
...
export const confirmPayment = async (req,res) => {
  try {
    ...
    if (!sessionId) return res.status(400).json({ success: false, message: "Missing sessionId"
     ...
     // Try to find the appointment by sessionId first
...
// to get the total number of user register to our account
export async function getRegisteredUsersCount(req, res)  {
  try {
    ...
    console.error("getRegisteredUsersCount error:", err);
    return res.status(500).json({
      success: false,
      message: "Server error"
    });

...
// Helper Functions
// Convert time string like "2:30 PM" to total minutes from midnight.
// and return AM to PM time in 24 hours format
...
// Remove duplicates and sort time slots for each date in the schedule and return the slot filter by time
...
// this function accept the object from the JSON string
...
// this function will convert the doctor data into plain text.
...
// Check existing doctor
  ...
  if(req.file?.path) {
    ...
    imageUrl = uploaded?.secure_url || uploaded?.public_id || imageUrl;
  ...
  // create Doctor
    ...
    if(!secret) {
      console.warn("JWT_SECRET is not define");
    ...
    const token = jwt.sign({id: doc._id.toString(), email: doc.email,
      role:"doctor"
    }, secret, {expiresIn:"7d"});
    ...
    console.log("CreateDoctor error:", err);
    return res.status(500).json({
      success:false,
...
const docs = await Doctor.aggregate([
    ...
    const total = await Doctor.countDocuments(match); // Total number of doctors matching the criteria.
...
// to get particular doctor by ID
...
// to update the particular doctor ID or field
...
// To make the doctor available or unavailable for the appointment
...
// To login the Doctor by email and password and generate JWT token for authentication

...
import { homeDoctorsStyles, iconSize } from '../assets/dummyStyles.js'
...
// To fetch doctor from the server side
  useEffect(() => {
    let mounted = true;
    async function load() {
      setLoading(true);
...
try {
  ...
  Medical Team
    ...
    <p className={homeDoctorsStyles.subtitle}>
      Book Appointment Quickly with our Specialists.
    </p>
    </div>
      {/* error / retry */}
        {error ? (
...
>
      ...
      <Medal className={`${iconSize?.small || ""} h-4`} />
        <span>
  {doctor.experience ? `${doctor.experience} years Experience` : "Experience N/A"}
</span>
      </div>
      </div>
      ...
      <div className={homeDoctorsStyles.buttonContainer}>
        ...
        doctor: doctor.raw || doctor
          ...
          <button disabled className={homeDoctorsStyles.buttonUnavailable}>
            <MousePointerOff className="w-5 h-5"/>
            Not Available
          </button>
         )}
        </div> 

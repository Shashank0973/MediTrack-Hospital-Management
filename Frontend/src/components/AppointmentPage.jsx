...
import { CheckCircle, Bell, Clock, XCircle, Wallet, CreditCard, CalendarDays, } from "lucide-react";
...
// This function will give response as YYYY-MM-DD with slots offset in hours
...
// to fetch doctor Appointments
  ...
  console.log("✅ Service API Response:", resp.data);
  ...
  if (params.get("service_payment") === "Paid") {

    const bookingData = JSON.parse(localStorage.getItem("serviceBooking"));
    ...
    const createBooking = async () => {
      try {
        ...
        if (!token) {
          console.error(" No token while booking");
          return;
        }

        await API.post("/api/service-appointments", bookingData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
...
// To get the appointment detail
  const appointmentData = useMemo(() => {
    return doctorAppts
      .map((a) => {
        ...
        const image =
  a.doctorImage?.url ||   // ✅ FIRST priority
  doctorObj.imageUrl ||
  doctorObj.image ||
  doctorObj.avatar ||
  "";
      ...
      <div className={appointmentPageStyles.maxWidthContainer}>
        ...
        {!loadingDoctors && appointmentData.length === 0 && (
          <div className={appointmentPageStyles.emptyStateText}>
            You have no doctor appointments.
          </div>
        )}
        ...
        <div className={appointmentPageStyles.doctorGrid}>
           {appointmentData.map((item)=> (
            ...
            <div className={cardStyles.doctorImageContainer}>
              <img src={item.image || "/doctor.png"} alt={item.doctor} className={cardStyles.image} loading="lazy"
              />
              </div>
            <h2 className={cardStyles.doctorName}>{item.doctor}
              </h2>
             <div className={cardStyles.specialization}>{item.specialization}{" "}
  ...
  <div className={cardStyles.rescheduledText}>
    Rescheduled to{" "}
    <span className={cardStyles.rescheduledDate}>
